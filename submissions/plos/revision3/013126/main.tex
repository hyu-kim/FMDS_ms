...
Next, the representation points were iteratively adjusted so that the $p$-value calculated in 2D space converged to the original PERMANOVA $p$-value.
...
\centering
\caption{
{\bf PERMANOVA test results for group differences in semi-synthetic datasets using the original structure ($p_\mathbf{x}$) and the principal coordinates analysis representation ($p_\mathbf{z}$). The datasets were generated using SparseDOSSA \cite{Ma21} \hl{(section ``Semisynthetic data'' of Methods).}}}
...
Under this rule, the algorithm converged in fewer than 30 iterations when $\lambda\geq0.2$ (Fig \ref{fig:sim_iter}B). 
...
{\bf PERMANOVA $p$-values from $F$-MDS representations of semisynthetic datasets.}
