...
Current Address: \#\!a Global Manufacturing Sciences and Technology, Sanofi, Framingham, MA 01701, USA
\#\!b Oak Ridge Institute for Science and Education, Department of Energy, Oak Ridge, TN 37830, USA 
...
Next, the representation points were iteratively adjusted so that the $p$-value calculated in 2D space \DIFdelbegin \DIFdel{converged }\DIFdelend \DIFaddbegin \hl{matches} \DIFaddend to the original PERMANOVA $p$-value.
...
Datasets are publicly available on Dryad at https://doi.org/10.5061/dryad.vmcvdnd3x \cite{Kim25}.
...
When training continued without a stopping rule, $p_\mathbf{z}$-values oscillated between 0 and 1, with a higher oscillation rate at larger $\lambda$ (\nameref{fig:sim_fmds_track}A). 
\DIFdelbegin \DIFdel{The stopping rule was therefore defined to trigger once $p_\mathbf{z}$ approached $p_\mathbf{x}$ closely and their difference was minimized}\DIFdelend \DIFdelend \DIFaddbegin \DIFadd{Therefore, we specified a bounded region, $|p_\mathbf{z} - p_\mathbf{x}| < 0.05$, as the stopping criterion for terminating the majorization algorithm }\DIFaddend  (\nameref{sec:app2}). 
Under this rule, the algorithm \DIFdelbegin \DIFdel{converged }\DIFdelend \DIFaddbegin \hl{stopped} \DIFaddend in fewer than 30 iterations when $\lambda\geq0.2$ (Fig \ref{fig:sim_iter}B). 
Repeating the experiment across triplicate simulated datasets of varying sizes confirmed that $\lambda=0.15$ was the minimal value required for \DIFdelbegin \DIFdel{convergence and optimization }\DIFdelend \DIFaddbegin \hl{the termination} \DIFaddend (\nameref{fig:sim_fmds_track}B). To preserve convexity of the majorization form of $O_{\rm FMDS}(\mathbf{Z})$ with a positive quadratic coefficient, the hyperparameter was constrained to values not exceeding unity (Fig \ref{fig:sim_iter}C, \nameref{sec:app2}).


\begin{figure}[h!]
    \centering
    \includegraphics[width=5.2in]{submissions/plos/figures/Fig2_rev.png}
    \caption{\textbf{Majorization algorithm optimizes the \textit{F}-informed MDS objective function.} (A) Changes in each term comprising the objective function (Eq \ref{eq:fmds}), including raw stress and confirmatory terms, are plotted against the training epochs for different hyperparameter values $\lambda$. A semisynthetic dataset of $N=200$, replicate 1, was generated using SparseDOSSA \cite{Ma21} (see the ``Semisynthetic data'' section of the Methods). (B) The PERMANOVA $p$-value under two-dimensional representation ($p_\mathbf{z}$) is plotted against epoch and $\lambda$ \DIFaddbeginFL \hl{until the stopping criteria are met}\DIFaddendFL . (C) Number of epochs until \DIFdelbegin \DIFdel{convergence}\DIFdelend \DIFaddbegin \hl{the termination} \DIFaddend is plotted against the hyperparameter, ranging between 0.2 and 1. Error bars indicate the standard deviation of triplicates.}
    \label{fig:sim_iter}
\end{figure}


After confirming the \DIFdelbegin \DIFdel{convergence }\DIFdelend \DIFaddbegin \hl{minimization} \DIFaddend of the objective with different hyperparameters, we next asked whether these values for $F$-MDS impacted its 2D representation. The visualizations were assessed using Shepard diagram and Stress, two quality metrics commonly used to evaluate how well an MDS representation preserves the distance structure from the original dimension (see \nameref{sec:methods}). Supervised MDS (superMDS \cite{witten11}) was chosen as a benchmark ordination method because its objective function is similarly controlled by a single hyperparameter $\alpha$ within the same range $[0,1]$, ensuring \DIFdelbegin \DIFdel{convergence }\DIFdelend \DIFaddbegin \hl{performance} \DIFaddend of its majorization algorithm. Shepard diagram analysis of these MDS methods showed that pairwise Euclidean distances deviated from the original structure in the two-dimensional representation to a greater extent when a nonzero hyperparameter was used. Interestingly, however, the change was more subtle in $F$-MDS (Fig \ref{fig:mds_eval_sim}A, \nameref{fig:sim_fmds_smds}A). 
...
Based on the hyperparameter $\lambda$'s influence on the MDS configuration, a grid search was conducted to identify an optimal value. Prior analysis demonstrated a faster $F$-MDS \DIFdelbegin \DIFdel{convergence }\DIFdelend \DIFaddbegin \hl{optimization} \DIFaddend with larger $\lambda$ (Fig \ref{fig:sim_iter}C). To balance the \DIFdelbegin \DIFdel{convergence rate }\DIFdelend \DIFaddbegin \DIFadd{epoch number }\DIFaddend against structural deviation while mitigating bias across the metrics, we defined an objective function $f_{\rm obj}(\lambda)$ via min-max normalization (\nameref{sec:app3_rev}). The loss function was evaluated over $\lambda$ between 0.15 and 1, where the \DIFdelbegin \DIFdel{convergence }\DIFdelend \DIFaddbegin \hl{performance} \DIFaddend was assured (\nameref{fig:fmds_gridsearch}A). Across semisynthetic datasets, the hyperparameter $\lambda$ that minimized $f_{\rm obj}(\lambda)$ was greater than or equal to 0.5 ($0.73\pm0.19$, mean $\pm$ s.d.; see \nameref{fig:fmds_gridsearch}B).
...
\label{sec:app1}
{\bf $F$-MDS objective function and mapping between $F$-ratios.}

\paragraph*{S2 Appendix.}
\label{sec:app2}
{\bf Majorization-Minimization algorithm.}

\paragraph*{S3 Appendix.}
\label{sec:app3_rev}
{\bf Hyperparameter selection procedure.}
...
{\bf Visualization of a semisynthetic dataset using SparseDOSSA.}

\paragraph*{S2 Fig.}
\label{fig:sim_fmds_track}
{\bf \DIFaddbegin \DIFadd{Stabiliity and trajectories of }\DIFaddend PERMANOVA $p$-values from $F$-MDS representations\DIFdelbegin \DIFdel{of semisynthetic datasets}\DIFdelend .}
