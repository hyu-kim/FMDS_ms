...
Repeating the experiment across triplicate simulated datasets of varying sizes confirmed that $\lambda=0.15$ was the minimal value required for convergence and optimization (\nameref{fig:sim_fmds_track}B).
\DIFaddbegin \DIFadd{Herein, ``convergence'' refers to satisfying the distributional matching criterion where $p_\mathbf{z}$ is close to $p_\mathbf{x}$, rather than asymptotic gradient zeroing. 
}\DIFaddend To preserve convexity of the majorization form of $O_{\rm FMDS}(\mathbf{Z})$ with a positive quadratic coefficient, the hyperparameter was constrained to values not exceeding unity (Fig \ref{fig:sim_iter}C, \nameref{sec:app2}).


\begin{figure}[h!]
    \centering
    \includegraphics[width=5.2in]{submissions/plos/figures/Fig2_rev.png}
    \caption{\textbf{Majorization algorithm optimizes the \textit{F}-informed MDS objective function.} (A) Changes in each term comprising the objective function (Eq \ref{eq:fmds}), including raw stress and confirmatory terms, are plotted against the training epochs for different hyperparameter values $\lambda$. A semisynthetic dataset of $N=200$, replicate 1, was generated using SparseDOSSA \cite{Ma21} (see the ``Semisynthetic data'' section of the Methods). (B) The PERMANOVA $p$-value under two-dimensional representation ($p_\mathbf{z}$) is plotted against epoch and $\lambda$ \DIFaddbeginFL \hl{until the stopping criteria are met}\DIFaddendFL . (C) Number of epochs until convergence is plotted against the hyperparameter, ranging between 0.2 and 1. Error bars indicate the standard deviation of triplicates.}
    \label{fig:sim_iter}
\end{figure}
