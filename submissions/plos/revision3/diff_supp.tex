...
\begin{figure}[ht]
    ...
    \caption*{\small Figure: Performance of self-supervised learning classifier by 50 training epochs. (A) Validation accuracy and (B) validation loss measured by categorical cross-entropy using bacterial community data represented as an image using PopPhy-CNN.}
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6.5in]{submissions/plos/figures/Fig_S2_rev2.pdf}
%DIFDELCMD <     \caption[PERMANOVA $p$-values from $F$-MDS representations of semisynthetic datasets.]{%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=6.5in]{submissions/plos/figures/Fig_S2_rev3.pdf}
    \caption[Stabiliity and trajectories of PERMANOVA $p$-values from $F$-MDS representations.]{\DIFaddFL{Stabiliity of }\DIFaddendFL PERMANOVA \DIFdelbeginFL \DIFdelFL{statistical significances }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p$-values }\DIFaddendFL from $F$\DIFdelbeginFL \DIFdelFL{-informed MDS }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-MDS }\DIFaddendFL representations\DIFdelbeginFL \DIFdelFL{using semisynthetic datasets}\DIFdelendFL . (A) \DIFdelbeginFL \DIFdelFL{Changes in the $p$-values over the number }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Trajectories }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{$F$-MDS training }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$p_z$ across 50 }\DIFaddendFL epochs \DIFdelbeginFL \DIFdelFL{without stopping rule}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for $\lambda = 0.2, 0.5,$ and $0.8$}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Red dashed lines indicate the bounded region of validity }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{B}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$|p_z - p_x| < 0.05$}\DIFaddendFL )\DIFdelbeginFL \DIFdelFL{After applying }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Pink points highlight where }\DIFaddendFL the stopping rule \DIFaddbeginFL \DIFaddFL{triggers}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{capturing the optimal statistical alignment before further oscillations occur. (B) Trajectories of }\DIFaddendFL $p$-values \DIFdelbeginFL \DIFdelFL{were }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with the stopping rule ($|p_z - p_x| < 0.05$). Heatmaps show $p_z$ values }\DIFaddendFL plotted against \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL training epochs for \DIFdelbeginFL \DIFdelFL{every hyperparameter $\lambda$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{hyperparameters $\lambda \in [0,1]$}\DIFaddendFL . Triplicate datasets \DIFdelbeginFL \DIFdelFL{with size $N=50,100,200,500$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of varying sizes ($N = 50, 100, 200, 500$) }\DIFaddendFL were evaluated \DIFaddbeginFL \DIFaddFL{to demonstrate consistent stabilization across conditions}\DIFaddendFL .}
...
\begin{figure}
    \centering
    \includegraphics[width=6.5in]{submissions/plos/figures/Fig_S4_rev2.pdf}
