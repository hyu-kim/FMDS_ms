...
\section*{Abstract}
Multidimensional scaling (MDS) is a \DIFaddbegin \DIFadd{widely used }\DIFaddend dimensionality reduction technique \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend microbial ecology data analysis that \DIFdelbegin \DIFdel{represents }\DIFdelend \DIFaddbegin \DIFadd{captures }\DIFaddend the multivariate structure \DIFaddbegin \DIFadd{of the data }\DIFaddend while preserving pairwise distances between samples. While \DIFdelbegin \DIFdel{its improvement has }\DIFdelend \DIFaddbegin \DIFadd{improvements in MDS have }\DIFaddend enhanced the ability to reveal \DIFdelbegin \DIFdel{data patterns by sample groups}\DIFdelend \DIFaddbegin \DIFadd{group-specific data patterns}\DIFaddend , these MDS-based methods require prior assumptions for inference, limiting their application in general microbiome analysis.
In this study, we introduce a new MDS-based ordination \DIFaddbegin \DIFadd{method}\DIFaddend , ``$F$-informed MDS,'' which configures the data distribution based on the $F$-statistic, the ratio of dispersion between groups sharing common and different characteristics. Using \DIFdelbegin \DIFdel{simulated compositional }\DIFdelend \DIFaddbegin \DIFadd{semisynthetic }\DIFaddend datasets, we demonstrate that the proposed method is robust to hyperparameter selection while maintaining statistical significance throughout the ordination process.
...
Our approach offers a well-founded refinement of MDS that aligns with statistical test results, which can be beneficial for broader \DIFdelbegin \DIFdel{compositional }\DIFdelend \DIFaddbegin \DIFadd{multidimensional }\DIFaddend data analyses in microbiology and ecology. This new visualization tool can be incorporated into standard microbiome data analyses.


\section*{Author summary}
Multidimensional scaling (MDS), also known as principal coordinate analysis, is a fundamental step in exploratory data analysis for interpreting microbial community samples processed via high-throughput sequencing. The interpretation of MDS results often involves linking patterns obtained from MDS with experimental treatments applied to the samples, such as environmental conditions or host phenotypes. However, retaining these patterns during ordination is not always guaranteed, as MDS itself does not consider group information during its learning process. This limitation reduces the effectiveness of conventional MDS\DIFdelbegin \DIFdel{in exploratory analysis}\DIFdelend , particularly for general microbiome datasets, where maintaining meaningful biological patterns is crucial. To address this gap, we present a robust statistical framework designed to represent microbiome datasets \DIFaddbegin \DIFadd{in a lower dimension, }\DIFaddend while preserving hypothesis testing results for group differences in the original dimension. Our approach, which relies on sample dispersion measured by the $F$-statistic, ensures a more stable and reliable performance compared to existing ordination methods. By incorporating statistical rigor into the ordination process, our framework improves the visualization of microbial community data and allows configurations to be adjusted within reasonable limits. This advancement provides researchers with a more effective tool for analyzing and interpreting complex microbiome data, ultimately leading to insightful conclusions.
...
Exploratory analysis of microbiome data \cite{peterson24, Armstrong22}, after preprocessing the sequencing reads \DIFdelbegin \DIFdel{into compositional data }\DIFdelend \DIFaddbegin \DIFadd{with data normalization }\DIFaddend \cite{Lin20, Weiss17, McMurdie14, paulson13}, typically begins with visualizing the multivariate structure to identify data patterns, distinguish variations\DIFaddbegin \DIFadd{, }\DIFaddend or remove outliers.
...
The most commonly used ordination method in ecology \cite{Jeganathan21} is multidimensional scaling (MDS), an ordination technique that represents data in a \DIFdelbegin \DIFdel{lower dimensional }\DIFdelend \DIFaddbegin \DIFadd{lower-dimensional }\DIFaddend space (e.g., two-dimensional or 2D) while preserving its original distance structure (Fig \ref{fig:intro}A).
...
Notably, most ordination methods for ecological data analysis do not utilize data labels and instead rely on unlabeled or unweighted distances in their computations. \DIFdelbegin \DIFdel{Concurrently}\DIFdelend \DIFaddbegin \DIFadd{To test for structural change and to correlate results with treatment effects}\DIFaddend , an additional and independent differential abundance analysis is required \DIFdelbegin \DIFdel{to test for structural change and to correlate with treatment }\DIFdelend \cite{peterson24, Armstrong22}.
While metric MDS can \DIFdelbegin \DIFdel{still be used to detect the }\DIFdelend \DIFaddbegin \DIFadd{detect some }\DIFaddend changes \cite{Borg05d}, its low-dimensional representation does not always capture every underlying \DIFdelbegin \DIFdel{pattern, even }\DIFdelend \DIFaddbegin \DIFadd{pattern---even }\DIFaddend when statistical validation confirms the \DIFaddbegin \DIFadd{presence of such a }\DIFaddend pattern in the original space. \DIFdelbegin \DIFdel{To address such cases}\DIFdelend \DIFaddbegin \DIFadd{A pattern is pronounced in the low-dimensional embedding only if the distance structures between groups are sufficiently distinct to allow a clear visualization. When this is not the case}\DIFaddend , scree plot analysis \DIFdelbegin \DIFdel{is performed }\DIFdelend \DIFaddbegin \DIFadd{can be used }\DIFaddend to determine the dimensionality at which the pattern \DIFdelbegin \DIFdel{information was preserved}\DIFdelend \DIFaddbegin \DIFadd{becomes visible}\DIFaddend . However, \DIFdelbegin \DIFdel{if more than three dimensions are required, effectively visualizing the multivariate data becomes impractical}\DIFdelend \DIFaddbegin \DIFadd{a scree plot does not indicate how the samples are distributed within this reduced dimension. Moreover, if the required dimensionality exceeds three, metric MDS cannot effectively visualize the difference between groups}\DIFaddend .

This limitation has driven the development of \DIFdelbegin \DIFdel{enhanced }\DIFdelend \DIFaddbegin \DIFadd{MDS-based }\DIFaddend methods that incorporate additional contextual information to improve pattern detection. One approach, broadly referred to as confirmatory MDS \cite{borg97b, ding18}, differentiates between sample groups by incorporating labels \cite{cox93, yang18}. This method is enabled by hyperparameter adjustments that optimize \DIFaddbegin \DIFadd{the }\DIFaddend objective function \cite{yang18, Wang22, witten11}.
%DIF < where label-containing or -free terms are balanced \cite{yang18, Wang22, witten11}.
...
In this work, we propose a novel framework, $F$-informed MDS ($F$-MDS), designed to visualize statistical differences within an exploratory MDS setting by leveraging the $F$-statistic \DIFaddbegin \DIFadd{(Fig \ref{fig:intro}B)}\DIFaddend . 
Our approach aims to introduce minimal perturbations to the \DIFdelbegin \DIFdel{original }\DIFdelend \DIFaddbegin \DIFadd{metric }\DIFaddend MDS representation, preserving the global distance structure as much as possible. 
\DIFaddbegin \DIFadd{We applied the perturbation only to datasets for which metric MDS failed to detect a statistically significant group-wise difference under the original data structure.
}\DIFaddend This generalizes previous label-based and model-based MDS methods by eliminating the need for prior assumptions about group structures or their differences. 
...
We consider a balanced design where an $i$-th sample, denoted as $\mathbf{x}_i \in \bbr^{S}$, is $S$-dimensional and associated to a discrete label \DIFdelbegin \DIFdel{$y_i \in \{0,1, \cdots r \}$ }\DIFdelend \DIFaddbegin \DIFadd{$y_i \in \{1,2, \cdots,G \}$, representing $G$ groups, }\DIFaddend for every index \DIFdelbegin \DIFdel{$i=1,\cdots N$ }\DIFdelend \DIFaddbegin \DIFadd{$i=1,2,\cdots, N$ }\DIFaddend (the number of samples, Fig \ref{fig:intro}A). 
\DIFdelbegin \DIFdel{A scalar, pairwise distance can be defined }\DIFdelend \DIFaddbegin \DIFadd{The pairwise distance }\DIFaddend between $\mathbf{x}_i$ and $\mathbf{x}_j$, denoted \DIFdelbegin \DIFdel{as $d_{ij}\in \bbr_{+}^{N}$, by using a }\DIFdelend \DIFaddbegin \DIFadd{$d_{ij}\in \bbr_{+}$, is computed using an arbitrary }\DIFaddend dissimilarity metric, \DIFdelbegin \DIFdel{e.g., }\DIFdelend \DIFaddbegin \DIFadd{such as }\DIFaddend Euclidean, Bray-Curtis \cite{Bray57}\DIFaddbegin \DIFadd{, }\DIFaddend or UniFrac \cite{lozupone05}. 
Let \DIFdelbegin \DIFdel{us also denote }\DIFdelend $\mathbf{D} = [d_{ij}]$ \DIFdelbegin \DIFdel{the matrix or a set }\DIFdelend \DIFaddbegin \DIFadd{denote the matrix }\DIFaddend of the pairwise distances.

In the metric multidimensional scaling, \DIFdelbegin \DIFdel{a lower-dimensional }\DIFdelend \DIFaddbegin \DIFadd{the goal is to find a two-dimensional }\DIFaddend representation $\mathbf{Z} = (\mathbf{z}_1, \cdots \mathbf{z}_N)\in \bbr^{2 \times N}$ \DIFdelbegin \DIFdel{is sought }\DIFdelend that best preserves the original \DIFdelbegin \DIFdel{, }\DIFdelend distance structure $\mathbf{D}$. 
...
Known algorithms to minimize the raw stress include matrix decomposition via Nyström approximation \cite{Platt05}, divide-and-conquer \cite{Yang06, Qu15} for Euclidean distances, or more generally for non-convex \DIFdelbegin \DIFdel{function}\DIFdelend \DIFaddbegin \DIFadd{functions}\DIFaddend , greedy algorithms \cite{Demaine21} such as majorization \cite{deLeeuw88, borg97a} or stochastic gradient descent \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Kruskal64, zheng19}}\hskip0pt%DIFAUXCMD
.
%DIF <  a difference in the pairwise distance between the dimensions and summing the difference for all pairs, so-called 
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Kruskal64, zheng19, Fang25}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

While the metric MDS is \DIFaddbegin \DIFadd{an }\DIFaddend unsupervised learning and does not require the label set \DIFdelbegin \DIFdel{$\mathbf{y}=(y_1,\cdots ,y_N)$, a supervised version }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}\in\{0,1\}^N$, supervised versions }\DIFaddend of multidimensional scaling \DIFdelbegin \DIFdel{, e.g., }\DIFdelend \DIFaddbegin \DIFadd{such as }\DIFaddend SuperMDS \cite{witten11}, \DIFdelbegin \DIFdel{imposes an additional constraint }\DIFdelend \DIFaddbegin \DIFadd{impose additional constraints }\DIFaddend on the representation by class groups.
...
\end{split}
\end{equation}
where the confirmatory term involves the \DIFdelbegin \DIFdel{groups set, balanced by the hyperparameter $\alpha$ }\DIFdelend \DIFaddbegin \DIFadd{group labels.
The two terms are balanced by a hyperparameter $\alpha \in [0,1]$ }\DIFaddend that controls the degree of classification.  
...
%DIF <  \textcolor{red}{[Describe the outline of this section here.]}
\DIFdelbegin \DIFdel{In this section }\DIFdelend \DIFaddbegin \DIFadd{Here, }\DIFaddend we introduce a \DIFdelbegin \DIFdel{multivariate test statistic based on samples }\DIFdelend \DIFaddbegin \DIFadd{test statistic for multivariate data based on sample }\DIFaddend dispersion (i.e., \textit{F}-statistic) and propose a revised MDS \DIFdelbegin \DIFdel{that utilizes the test }\DIFdelend \DIFaddbegin \DIFadd{framework that utilizes this }\DIFaddend statistic.
...
Instead, a ``pseudo'' $F$-statistic has been defined \cite{clarke93}, by denoting $\epsilon_{ij}$ the \DIFdelbegin \DIFdel{$\1\{y_i=y_j\}$ (indicator function ) to write
}\DIFdelend \DIFaddbegin \DIFadd{indicator function $\1\{y_i=y_j\}$ to write
}\DIFaddend \begin{equation} \label{eq:pseudof}
F = \DIFdelbegin \DIFdel{\frac{\sum_{i,j}d_{ij}^2 - 2\sum_{i,j}\epsilon_{ij}d_{ij}^2}{2\sum_{i,j}\epsilon_{ij}d_{ij}^2}}\DIFdelend \DIFaddbegin \left(\DIFadd{\frac{\sum_{i,j}d_{ij}^2}{G\sum_{i,j}\epsilon_{ij}d_{ij}^2}-1}\right)\DIFaddend \cdot \DIFdelbegin \DIFdel{(N-2)}\DIFdelend \DIFaddbegin \DIFadd{\frac{N-G}{G-1}}\DIFaddend .
\end{equation} 

\DIFdelbegin \DIFdel{Using }\DIFdelend \DIFaddbegin \DIFadd{The empirical distribution of }\DIFaddend the pseudo-$F$ \DIFdelbegin \DIFdel{'s, an empirical distribution can be constructed by permuting the labels for a large sized dataset }\DIFdelend \DIFaddbegin \DIFadd{statistic is approximated by label permutation for sufficiently large datasets }\DIFaddend \cite{holmes96, gijbels13}.
Specifically, \DIFdelbegin \DIFdel{a new $F$-ratio denoted as $F^{\pi}$ is calculated from the newly permuted label set $[y_i]^{\pi}$ and the calculation is repeated over }\DIFdelend $K$ independent permutations \DIFdelbegin \DIFdel{, $\pi_1 , \cdots ,\pi_K$}\DIFdelend \DIFaddbegin \DIFadd{$\Pi_1 , \cdots ,\Pi_K$ generate permuted label sets $[y_i^{\Pi_k}]_{i=1}^{N}$, from which the values $F^{\Pi_k}$ are computed}\DIFaddend . This pseudo-$F$-distribution \DIFdelbegin \DIFdel{yields }\DIFdelend \DIFaddbegin \DIFadd{is then used to obtain }\DIFaddend a $p$-value, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend procedure known as \DIFdelbegin \DIFdel{the }\DIFdelend permutational multivariate analysis of variance (PERMANOVA) \cite{anderson01}:
\begin{equation}
...
To incorporate hypothesis testing into MDS \DIFdelbegin \DIFdel{visual }\DIFdelend analysis, we take a weakly supervised approach using the following steps (Fig \ref{fig:intro}B).
...
\end{equation}
\DIFdelbegin \DIFdel{in which }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend the confirmatory term \DIFdelbegin \DIFdel{can minimize a difference }\DIFdelend \DIFaddbegin \DIFadd{minimizes the discrepancy }\DIFaddend between $p$-values from the \DIFaddbegin \DIFadd{permutation }\DIFaddend $F$-distributions \DIFdelbegin \DIFdel{represented at each dimensionality, i.e., }\DIFdelend \DIFaddbegin \DIFadd{obtained at each dimension, namely the }\DIFaddend original and the \DIFdelbegin \DIFdel{two-dimension (herein denote them }\DIFdelend \DIFaddbegin \DIFadd{two-dimensional representations (denoted here }\DIFaddend as $F_\mathbf{x}$ and $F_\mathbf{z}$, respectively).
\DIFdelbegin \DIFdel{In detail, after noticing }\DIFdelend \DIFaddbegin \DIFadd{Specifically, recognizing }\DIFaddend that the distributions \DIFdelbegin \DIFdel{can vary depending on the dimension, we compensated for the }\DIFdelend \DIFaddbegin \DIFadd{may depend on dimensionality, we account for this }\DIFaddend difference by introducing a local regression function $f_\mathbf{z}$, \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend maps between the two $F$-ratios, each \DIFdelbegin \DIFdel{`}\DIFdelend pre-ordered \DIFdelbegin \DIFdel{' by size based on }\DIFdelend \DIFaddbegin \DIFadd{by magnitude according to the }\DIFaddend permuted labels.
In other words, a local regression is performed using two ordered $F$-ratios, $(F_\mathbf{x}, F_\mathbf{z})$, where each element in the tuple is computed from two independent, permuted sets, \DIFdelbegin \DIFdel{$\mathbf{y}^{\pi_1}$ and $\mathbf{y}^{\pi_2}$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi_1}$ and $\mathbf{y}^{\Pi_2}$ }\DIFaddend (Algorithm \ref{alg:mapping}). Details on the mapping function and the derivation of an exact $O_{\rm FMDS}$ are \DIFdelbegin \DIFdel{explained }\DIFdelend \DIFaddbegin \DIFadd{given }\DIFaddend in \nameref{sec:app1}.
%DIF < Additionally introduced in Eq \ref{eq:fmds} is the regression function, 
%DIF <  In other words, by denoting two independent permutations as $\pi_1,\,\pi_2$, the two $F$-ratios are written from Eq \ref{eq:pseudof} as
%DIF <  \begin{align}
%DIF <  F_\mathbf{x}^{\pi_1} &= 
%DIF <  \left(\frac{\sum_{i,j}d_{ij}^2}{2\sum_{i,j}d_{ij}^2\,\1\{y_i^{\pi_1} = y_j^{\pi_1}\}} -1 \right) \cdot (N-2), \label{eq:F_x_perm} \\
%DIF <  F^{\pi_2}_\mathbf{z} &= 
%DIF <  \left(\frac{\sum_{i,j}\|\mathbf{z}_i - \mathbf{z}_j\|_2^2}{2\sum_{i,j}\|\mathbf{z}_i - \mathbf{z}_j\|_2^2\,\1\{y_i^{\pi_1} = y_j^{\pi_2}\}} -1 \right) \cdot (N-2).
%DIF <  \label{eq:F_z_perm}
%DIF <  \end{align} 
...
\For{$1\leq i \leq 999$}
    \State\DIFdelbegin \DIFdel{$\mathbf{y}^{\pi_1} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi_1} \Leftarrow$ }\DIFaddend random permutation on $\mathbf{y}$
    \State\DIFdelbegin \DIFdel{$\mathbf{y}^{\pi_2} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi_2} \Leftarrow$ }\DIFaddend random permutation on $\mathbf{y}$
    \State\DIFdelbegin \DIFdel{$F^{\pi_1}_\mathbf{x} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi_1}_\mathbf{x} \Leftarrow$ }\DIFaddend pseudo-$F$ using $\mathbf{D}$ and \DIFdelbegin \DIFdel{$\mathbf{y}^{\pi_1}$
    }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi_1}$
    }\DIFaddend \State\DIFdelbegin \DIFdel{$F^{\pi_2}_\mathbf{z} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi_2}_\mathbf{z} \Leftarrow$ }\DIFaddend pseudo-$F$ using $\mathbf{Z}$ and \DIFdelbegin \DIFdel{$\mathbf{y}^{\pi_2}$
    }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi_2}$
    }\DIFaddend \State$\mathcal{L}_\mathbf{x} \Leftarrow$ append \DIFdelbegin \DIFdel{$F^{\pi_1}_\mathbf{x}$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi_1}_\mathbf{x}$ }\DIFaddend to $\mathcal{L}_\mathbf{x}$
    \State$\mathcal{L}_\mathbf{z} \Leftarrow$ append \DIFdelbegin \DIFdel{$F^{\pi_2}_\mathbf{z}$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi_2}_\mathbf{z}$ }\DIFaddend to $\mathcal{L}_\mathbf{z}$
\EndFor
...
\DIFdelbegin \DIFdel{In our problem setting, we implemented }\DIFdelend \DIFaddbegin \DIFadd{We implement }\DIFaddend the majorization (or Majorization-Minimization) algorithm to minimize the $F$-MDS objective (Eq \ref{eq:fmds}). First, an exact form of $O_{\rm FMDS}(\mathbf{Z})$ \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend derived to match the physical dimension of each term (\nameref{sec:app1}),
\begin{equation}
    O_{\rm FMDS}(\mathbf{Z}) = \DIFaddbegin \DIFadd{\frac{1}{N}}\DIFaddend \sum_{i,j} (d_{ij} - \| \mathbf{z}_i - \mathbf{z}_j \|_2 )^2 + \lambda\left|\sum_{i,j} \left[1- \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{G}\DIFaddend \epsilon_{ij} \left(1+\DIFdelbegin \DIFdel{\frac{f_\mathbf{z}(F_\mathbf{x})}{N-2}}\DIFdelend \DIFaddbegin \DIFadd{\frac{G-1}{N-G}f_\mathbf{z}(F_\mathbf{x})}\DIFaddend \right)\right] \|\mathbf{z}_i - \mathbf{z}_j\|_2^2\right|.
\end{equation}
Next, as similarly described in \cite{borg05c, witten11}, the majorization is applied to seek an optimal $k$-th point for every $k=1,\cdots N$, while keeping other points \DIFdelbegin \DIFdel{except for $\mathbf{z}_{k}$ fixed,
    ...
    \label{eq:seek_z}
\end{equation}
It can be analytically shown that Eq \ref{eq:seek_z} \DIFdelbegin \DIFdel{expression }\DIFdelend can be majorized \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a quadratic expression in \DIFdelbegin \DIFdel{terms of }\DIFdelend $\mathbf{z}_k$, \DIFdelbegin \DIFdel{where the quadratic }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend is further minimized by taking a derivative with respect to each element of $\mathbf{z}_k$ (denoted as \DIFdelbegin \DIFdel{${z}_{ks},\ s=1,\cdots ,r$}\DIFdelend \DIFaddbegin \DIFadd{${z}_{ks},\ s=1,2$}\DIFaddend ). An update rule \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend established by solving for ${z}_{ks}$ after setting the derivative to zero. \DIFdelbegin \DIFdel{Detailed derivation }\DIFdelend \DIFaddbegin \DIFadd{It should be noted that the update is applied exclusively to the representation where its group-wise difference is not identified ($p_\mathbf{z}\gg p_\mathbf{x}$), which is significant under the original structure ($p_\mathbf{x} < 0.1$).  Detailed derivations }\DIFaddend of the expression and the update rule for $\mathbf{Z}$ are described in \nameref{sec:app2}, and the whole procedure is summarized in Algorithm \ref{alg:mm}.

\begin{algorithm}[h]
\caption{\DIFdelbegin \DIFdel{Majorization }\DIFdelend \DIFaddbegin \DIFadd{Update rule }\DIFaddend for computing $F$-MDS.}
\label{alg:mm}
\begin{algorithmic}[1]
\DIFdelbegin %DIFDELCMD < \Require{
%DIFDELCMD < \begin{tabular}{@{}ll}
...
\For{$1\leq k \leq N$}
    ...
    \State$\mathbf{z}_k \Leftarrow$ update $\mathbf{z}_k$ with $\delta(\mathbf{z})$ and $f_\mathbf{z}(F_\mathbf{x})$  (Equation \DIFdelbegin \DIFdel{S18}\DIFdelend \DIFaddbegin \DIFadd{S16}\DIFaddend )
\EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
...
%DIFDELCMD < %%%
\DIFdel{Specifically for a binary dataset, triplicates of 100 samples were randomly drawn from truncated normal distributions \mbox{%DIFAUXCMD
...
\DIFdelend \DIFaddbegin \DIFadd{The behavior of $F$-informed MDS is illustrated using simulation studies with semisynthetic datasets designed to mimic a microbiome community. Among eleven available simulation packages \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{A compositionality characteristic was additionally represented by applying a total sum scaling to the multivariate data, i.e.}\DIFdelend \DIFaddbegin \DIFadd{where $\pi_s$ is the probability that feature $s$ is absent, and $\mu_s$ and $\sigma_s$ are the mean and standard deviation of the log-transformed, nonzero feature abundance, respectively. In addition, dependency among features in SparseDOSSA is indirectly handled by the copula parameter $\Omega\in\cR^{S\times S}$. This parameter describes a latent multivariate Gaussian variable that is compared with $\pi_s$ to determine whether the feature is present.
}

\DIFadd{In the simulation studies, SparseDOSSA parameters were estimated by fitting the model to shotgun-sequenced metagenomes of healthy human stool (``stool'' \mbox{%DIFAUXCMD
\cite{Lloyd17}}\hskip0pt%DIFAUXCMD
)}\DIFaddend , \DIFdelbegin \DIFdel{dividing a sample components with their sum, }\DIFdelend \DIFaddbegin \DIFadd{followed by mean adjustments. The training set consists of 239 stool samples, each comprising 332 microbial features. For the binary-group simulation (i.e., $y=1,2$), two balanced semisynthetic datasets were generated using the fitted stool parameters, with all but one feature mean differently adjusted for each dataset. Specifically, a feature $s_1$ with the smallest ``effective'' variance $\sigma_{{\rm eff}}^2$ was selected (Eq \ref{eq:effective_sigma}); an offset value of $\pm 5$ for each group was then added to that feature's mean, i.e., $\mu_s(s=s_1)$ (Eq \ref{eq:binary}). 
}\DIFaddend \begin{equation}
...
\DIFadd{Similarly, for ternary-group simulation ($y=1,2,3$), three balanced semisynthetic datasets were generated using the fitted SparseDOSSA parameters after differently adjusting the mean values of five features ($s_i,\,i=1,\cdots,5$) (Eq \ref{eq:ternery}). Again, the features were selected from the 332 features to have the smallest effective variances $\sigma_{{\rm eff}}^2$. 
}\DIFaddend \begin{equation}
...
For the algal-associated microbiome, the dataset consisted 36 balanced microbial community samples cultured with or without the microbial host (alga \textit{Phaeodactylum tricornutum}). The community comprised 72 bacterial taxa as identified by amplicon sequence variants (ASV) of the 16S rRNA gene, PCR-amplified. ASV counts were obtained through \DIFaddbegin \DIFadd{the }\DIFaddend Illumina MiSeq system and subsequently converted into relative abundances using cumulative sum scaling (CSS) \cite{paulson13}. A phylogeny-based metric (weighted Unifrac \cite{lozupone07}) \DIFdelbegin \DIFdel{and compositional data were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend used to calculate pairwise \DIFdelbegin \DIFdel{distance between samples, forming the distance matrix $\mathbf{D}$}\DIFdelend \DIFaddbegin \DIFadd{distances between samples}\DIFaddend .
For the human gut microbiome, two datasets generated from shotgun metagenomic sequencing were retrieved from a publicly available repository \cite{Pasolli16}, where reads were merged at the genus level \cite{reiman20}. Detailed reproduction and processing steps for these datasets are described in \DIFdelbegin %DIFDELCMD < \nameref{sec:app3}%%%
\DIFdelend \DIFaddbegin \nameref{sec:app4_rev}\DIFaddend .

\paragraph*{Performance evaluation metrics.}
To quantify the performance of dimension reductions, four known \cite{Espadoto21} and two new quality metrics were defined and jointly used. Trustworthiness and continuity, among the first four metrics, measure how closely neighborhood points are preserved from the original ($S$-) to \DIFaddbegin \DIFadd{a }\DIFaddend two-dimensional representation, or vice versa, defined in \cite{Venna01}\DIFdelbegin \DIFdel{and expressed as
...
We first characterized the behavior of $F$-informed MDS by observing changes in the MDS representation across a range of hyperparameters. \DIFdelbegin \DIFdel{The changes were recorded after }\DIFdelend \DIFaddbegin \DIFadd{After }\DIFaddend each update of the representation $\mathbf{Z}$\DIFdelbegin \DIFdel{by computing its objective function , }\DIFdelend \DIFaddbegin \DIFadd{, the changes were recorded by computing the objective function }\DIFaddend $O_{\rm FMDS}(\mathbf{Z})$, and \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{PERMANOVA }\DIFaddend $p$-value \DIFdelbegin \DIFdel{from PERMANOVA testing of }\DIFdelend \DIFaddbegin \DIFadd{$p_\mathbf{z}$ from testing on }\DIFaddend $\mathbf{Z}$\DIFdelbegin \DIFdel{($p_\mathbf{z}$, Fig \ref{fig:intro})}\DIFdelend . The $p$-value was chosen as a tracking measure instead of the pseudo $F$-statistic because its distribution varied depending on the hyperparameter $\lambda$ (\nameref{sec:app1}). \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{To generate data, }\DIFaddend binary, balanced \DIFdelbegin \DIFdel{dataset of 100 random samples was simulated and normalized using total sum scaling, where each sample group followed a four-dimensional truncated normal distribution with the same covariance but different means }\DIFdelend \DIFaddbegin \DIFadd{datasets of sizes $N$ ranging from 50 to 500 were simulated using SparseDOSSA \mbox{%DIFAUXCMD
\cite{Ma21}}\hskip0pt%DIFAUXCMD
...
\begin{figure}[h!]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5.2in]{submissions/plos/figures/Fig2_v2.pdf}
%DIFDELCMD <     %%%
...
\DIFdelbegin \DIFdel{Additionally, since the iterations }\DIFdelend \DIFaddbegin 


\begin{figure}[h!]
    \centering
    \includegraphics[width=5.2in]{submissions/plos/figures/Fig3_rev.png}
    \caption{\textbf{\textit{\DIFaddFL{F}}\DIFaddFL{-informed MDS is robust to the choice of hyperparameters.}} \DIFaddFL{(A) Shepard plots comparing pairwise distances from $F$-informed MDS ($F$-MDS) and supervised MDS (superMDS) under both zero (metric MDS) and nonzero hyperparameter settings ($\lambda$, $\alpha=1$). Distances in the original high-dimensional space were computed using Bray-Curtis dissimilarity; distances in the two-dimensional embedding used Euclidean metric. (B) For each method, the Pearson correlation coefficient was calculated between original and embedded distances. (C) Normalized stress (Stress-1) was computed for each embedding. Analysis used semisynthetic datasets with $N=200$. Error bars indicate standard deviation across triplicate datasets.}}
    \label{fig:mds_eval_sim}
\end{figure}


\DIFadd{Additionally, we note that the number of training epochs }\DIFaddend required to compute $F$-MDS \DIFdelbegin \DIFdel{are minimized and reach a plateau at $\lambda\approx1$ }\DIFdelend \DIFaddbegin \DIFadd{is minimized at a higher values of $\lambda$ }\DIFaddend (Fig \ref{fig:sim_iter}C), \DIFdelbegin \DIFdel{we suggest that the majorization algorithm provides an effective strategy for hyperparameter selection, ensuring high-quality performance while reducing computational cost . 
}\DIFdelend \DIFaddbegin \DIFadd{To select the hyperparameter quantitatively, a grid search was conducted (see }\nameref{sec:app3_rev}\DIFadd{). We introduced an objective function $f_{\rm obj}(\lambda)$ that balances the increased computational cost against increased structural deviation, evaluating this function over $\lambda\in[0,1]$ (}\nameref{fig:fmds_gridsearch}\DIFadd{A). For all semisynthetic datasets, the hyperparameter $\lambda$ that minimized $f_{\rm obj}(\lambda)$ was greater than or equal to 0.5 ($0.78\pm0.17$, mean $\pm$ s.d.; see }\nameref{fig:fmds_gridsearch}\DIFadd{B). This result confirms that the performance of our selection framework is largely determined by the number of computation epochs. 
}\DIFaddend 

 
\DIFdelbegin %DIFDELCMD < \begin{figure}[h!]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics[width=5.2in]{submissions/plos/figures/Fig3_v2.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\textit{\DIFdelFL{F}}%DIFAUXCMD
\DIFdelFL{-informed MDS is robust to the choice of hyperparameters.}} %DIFAUXCMD
\DIFdelFL{(A) Shepard plots of pairwise distances using $F$-informed MDS ($F$-MDS) and supervised MDS (superMDS) with both zero (i.e., metric MDS) and nonzero hyperparameter settings ($\lambda=1$). For each dimensionality, distances were measured using weighted UniFrac (original) and Euclidean (2D). The pairwise distances from the original and two-dimensional representations were then used to calculate (B) their Pearson correlation coefficient and (C) normalized stress (Stress-1). Error bars represent the standard deviation of calculations from triplicate simulation datasets.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:mds_eval_sim}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD <  
%DIFDELCMD < %%%
\DIFdelend \subsection*{Using different quality metrics confirms the \DIFdelbegin \DIFdel{high }\DIFdelend \DIFaddbegin \DIFadd{consistent }\DIFaddend performance of $F$-MDS.}
...
After evaluating $F$-MDS using different quality metrics, we applied the method to understand how sample groups were differentiated in 2D representations\DIFdelbegin \DIFdel{of compositional datasets. The compositional }\DIFdelend \DIFaddbegin \DIFadd{. The semi-synthetic and bacterial community }\DIFaddend datasets described earlier were used for visualization, noting that their group differences were not distinguishable under 2D representations with metric MDS (\DIFdelbegin \DIFdel{$p_\mathbf{z}=0.645,\ 0.5$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathbf{z}\geq 0.5$}\DIFaddend ), whereas in the original dimension, they are statistically more distinct (\DIFdelbegin \DIFdel{$p_\mathbf{x}=0.004\ \text{and} <0.1$, respectively}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathbf{x}<0.1$}\DIFaddend ). To detect structural changes in these representations, visualizations were generated using metric MDS and $F$-MDS with varying hyperparameters, and the results were compared by calculating the inter-centroid distance and variance of each group.

For the simulated dataset, we observed a shift in group centroids when hyperparameter $\lambda$ increased from zero to one, with a unidirectional shift that separated the groups (Fig \ref{fig:vis}). The increase in inter-group centroid distances was more pronounced at lower hyperparameter values (\DIFdelbegin \DIFdel{$\lambda < 0.4$}\DIFdelend \DIFaddbegin \DIFadd{$\lambda < 0.2$}\DIFaddend ), with an increase rate \DIFdelbegin \DIFdel{of $0.28\pm 0.04\ (\lambda^{-1})$, compared to a slower rate at higher values ($0.07\pm 0.03\ \lambda^{-1},\ n=3$; }\DIFdelend \DIFaddbegin \DIFadd{1.9--25.7 fold higher than those at higher $\lambda$ values (}\DIFaddend \nameref{fig:fmds_rep_analysis}\DIFdelbegin \DIFdel{A}\DIFdelend ). The finding suggests that the re-arrangement of the MDS representation, constrained by the majorization, slowed down and stabilized at high $\lambda$. This is consistent with our earlier iteration number analysis (Fig \ref{fig:sim_iter}), where the \DIFdelbegin \DIFdel{iteration }\DIFdelend \DIFaddbegin \DIFadd{computation epoch }\DIFaddend number reached a minimum, satisfying the objective $p_\mathbf{z}\approx p_\mathbf{x}$ as the hyperparameter was reached unity. Additionally, we observed a steady decrease in each group's variance by \DIFdelbegin \DIFdel{approximately $\sim\!12-14\%$ }\DIFdelend \DIFaddbegin \DIFadd{$8.5\pm7.8\%$ }\DIFaddend as $\lambda$ increased from 0 and 1 (\nameref{fig:fmds_rep_analysis}\DIFdelbegin \DIFdel{B,C}\DIFdelend ), as measured from the eigendecomposition of their covariances. This indicates that $F$-MDS further differentiated the groups by reducing their respective variances. 
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=5.2in]{submissions/plos/figures/Fig6_rev.png}
    \DIFaddendFL \caption{\textbf{\textit{F}-informed MDS can visualize group-based data discrimination.} Two-dimensional representations of (A) a simulated \DIFdelbeginFL \DIFdelFL{compositional }\DIFdelendFL dataset and (B) an algal microbiome dataset, generated using metric MDS and $F$-informed MDS ($F$-MDS) with two hyperparameter settings ($\lambda=$ 0.5, 1). Ellipses of respective colors are drawn with a confidence level of 0.68.}
    \label{fig:vis}
\end{figure}
...
\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:ternery}, }\nameref{fig:sim_ternary_vis}\DIFaddend ). Similar to the earlier binary simulated dataset, the distribution was elliptically shaped, resulting in an indistinguishable representation in metric MDS (\DIFdelbegin \DIFdel{$p_\mathbf{z}=0.94$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathbf{z}=0.965$}\DIFaddend ), whereas the original structures are statistically different (\DIFdelbegin \DIFdel{$p_\mathbf{x}<0.001$}\DIFdelend \DIFaddbegin \DIFadd{$p_\mathbf{x}=0.053$}\DIFaddend ). Computing $F$-MDS with a hyperparameter $\lambda=0.5$ showed that the sample groups deviated from each other, making the differences more visible when ellipses with a 68\% confidence level were drawn (Fig \ref{fig:vis_multiclass}). While this deviation led to a statistically significant 2D representation ($p_\mathbf{z}=0.001$), the ternary class setting rendered the difference less pronounced compared to binary data representations.
...
\DIFdelendFL \DIFaddbeginFL \nameref{fig:sim_ternary_vis}\DIFaddendFL ). Ellipses of respective colors are drawn with a confidence level of 0.68.}
...
\section*{Discussion}
In this work, we proposed a weakly supervised multidimensional scaling method based on the $F$-ratio and characterized its representations by varying hyperparameters and the \DIFdelbegin \DIFdel{iteration }\DIFdelend \DIFaddbegin \DIFadd{epoch }\DIFaddend number of the majorization algorithm. Evaluations using simulated and bacterial community datasets showed that our $F$-informed MDS outperformed existing MDS-based and other dimension reduction methods in preserving local and global structures, as well as class-based information (e.g., statistical inference). The datasets used in this study represented cases where metric MDS failed to identify group differences through visualizations, partly due to the loss of such information during dimensionality reduction. This highlights the limitations of metric MDS in microbiome data analysis. Our dispersion-based approach expanded its applicability by introducing an additional constraint to adjust the MDS representation minimally. Since the adjusted representations significantly retain the sample distributions from metric MDS, this method remains applicable for visualizing microbial communities while enabling group-associated inferences directly from the visualizations.

One limitation of $F$-informed MDS, however, is that its computation relies on the initial configuration from metric MDS, and the computational cost increase with iterations of majorization and label permutations (a summary of method costs is provided in \DIFdelbegin %DIFDELCMD < \nameref{sec:app5}%%%
\DIFdel{). }\DIFdelend \DIFaddbegin \nameref{sec:app6_rev}\DIFadd{). Indeed, majorization for MDS has been known for its high computational cost \mbox{%DIFAUXCMD
\cite{Fang25}}\hskip0pt%DIFAUXCMD
, although its numerical convergence is guaranteed \mbox{%DIFAUXCMD
\cite{deleeuw24} }\hskip0pt%DIFAUXCMD
and we did not observe any stability issues in our simulation. }\DIFaddend Recent theoretical advances in computational algorithms, including majorization \cite{Streeter23} or (stochastic) gradient descent \cite{Demaine21, zheng19} open doors for improving our framework toward unrestricted initialization and more efficient iterations. Additionally, empirical observations suggest that $F$-MDS representations became less reliant on the choice of hyperparameter at higher values. Once the relationship between the two objective-constituting terms is clarified, it could lead to a viable strategy for selecting an optimal hyperparameter without requiring recursive procedures such as cross-validation.

In summary, the proposed $F$-MDS provides a useful tool for visualizing high-throughput microbiome data while simultaneously delivering statistical testing results. We anticipate that this method will be beneficial for broader applications in microbiology and ecology data analysis. The R implementation of $F$-MDS is available at https://\DIFdelbegin \DIFdel{github.com}\DIFdelend \DIFaddbegin \DIFadd{bioconductor.org}\DIFaddend /\DIFdelbegin \DIFdel{soob-kim}\DIFdelend \DIFaddbegin \DIFadd{packages}\DIFaddend /\DIFdelbegin \DIFdel{fmds}\DIFdelend \DIFaddbegin \DIFadd{FinfoMDS/}\DIFaddend .



\section*{Acknowledgments}
The authors acknowledge the MIT Office of Research Computing and Data for providing high performance computing resources that have contributed to the research results reported within this paper. We thank C. Belthangady and J.-Y. Lee for guidance in developing neural network models\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and R.D. Braatz for providing additional resources.
}\DIFaddend 
...
\label{sec:app1}
{\bf \DIFdelbegin \DIFdel{Mapping function for }\DIFdelend \DIFaddbegin \DIFadd{Objective function for computing }\DIFaddend $F$-MDS.}
...
\DIFdelbegin %DIFDELCMD < \label{sec:app3}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:app3_rev}
\DIFaddend {\bf \DIFaddbegin \DIFadd{Hyperparameter selection.}}

\paragraph*{\DIFadd{S4 Appendix.}}
\label{sec:app4_rev}
{\bf \DIFaddend Human gut microbiome dataset.}

\paragraph*{\DIFdelbegin \DIFdel{S4 }\DIFdelend \DIFaddbegin \DIFadd{S5 }\DIFaddend Appendix.}
\DIFdelbegin %DIFDELCMD < \label{sec:app4}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:app5_rev}
\DIFaddend {\bf Neural network model and architecture.}

\paragraph*{\DIFdelbegin \DIFdel{S5 }\DIFdelend \DIFaddbegin \DIFadd{S6 }\DIFaddend Appendix.}
\DIFdelbegin %DIFDELCMD < \label{sec:app5}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:app6_rev}
\DIFaddend {\bf Computational complexity.}
...
\DIFdel{-correlation plot }\DIFdelend \DIFaddbegin \DIFadd{Pairwise distance analyses }\DIFaddend from \DIFdelbegin \DIFdel{simulated dataset with seven }\DIFdelend \DIFaddbegin \DIFadd{$F$-informed MDS and supervised MDS.}}

\paragraph*{\DIFadd{S4 Fig.}}
\label{fig:fmds_gridsearch}
\textbf{\DIFadd{Hyperparameter selection using grid search method.}}

\paragraph*{\DIFadd{S5 Fig.}}
\label{fig:sim_eval_all}
\textbf{\DIFadd{Comparison of quality metrics of benchmark }\DIFaddend ordination methods \DIFaddbegin \DIFadd{on all datasets}\DIFaddend .}

\DIFdelbegin \paragraph*{\DIFdel{S4 Fig.}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \paragraph*{\DIFadd{S6 Fig.}}
\DIFaddend \label{fig:shepard_alga_all}
\textbf{Shepard plot from algal microbiome dataset with eight ordination methods.}

\DIFdelbegin \paragraph*{\DIFdel{S5 Fig.}}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \paragraph*{\DIFadd{S7 Fig.}}
\DIFaddend \label{fig:f_corr_alga_all}
\DIFdelbegin %DIFDELCMD < \textbf{%%%
\textit{\DIFdel{F}}%DIFAUXCMD
\DIFdel{-correlation plot from algal microbiome dataset.}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \textbf{\textit{\DIFadd{F}}\DIFadd{-correlation plot from algal microbiome dataset.}}
\DIFaddend 
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \paragraph*{\DIFadd{S8 Fig.}}
\DIFaddend \label{fig:fmds_rep_analysis}
\textbf{Cluster centroid and variances of $F$-MDS representations\DIFdelbegin \DIFdel{with simulated dataset}\DIFdelend .} 
\DIFaddbegin 
