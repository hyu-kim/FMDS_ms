...
%DIF -------
\usepackage{algorithm, algorithmicx, algpseudocode} %DIF > 
 %DIF > 
\usepackage{soul} %DIF > 
...
Current Address: \#\!a \DIFdelbegin \DIFdel{Department of Chemical Engineering, Massachusetts Institute of Technology, Cambridge, MA, USA %DIF <  change symbol to "\textcurrency a" if more than one current address note
}\DIFdelend \DIFaddbegin \DIFadd{Global Manufacturing Sciences and Technology, Sanofi, Framingham, MA 01707, USA
}\DIFaddend \#\!b Oak Ridge Institute for Science and Education, Department of Energy, Oak Ridge, TN \DIFaddbegin \DIFadd{37830}\DIFaddend , USA 
...
\begin{adjustwidth}{0in}{0in}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5.2in]{submissions/plos/figures/Fig1_rev.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=5.2in]{submissions/plos/figures/Fig1_rev2.png}
...
To incorporate hypothesis testing into MDS analysis, we \DIFdelbegin \DIFdel{take }\DIFdelend \DIFaddbegin \DIFadd{developed }\DIFaddend a weakly supervised \DIFdelbegin \DIFdel{approach using the following steps }\DIFdelend \DIFaddbegin \DIFadd{learning method }\DIFaddend (Fig \ref{fig:intro}B).
First, a two-dimensional representation \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend initialized by computing metric MDS \DIFdelbegin \DIFdel{, which is unsupervised and label-free}\DIFdelend \DIFaddbegin \DIFadd{(i.e., an unsupervised step)}\DIFaddend .
Next, the representation points \DIFdelbegin \DIFdel{are iteratively adjusted to ensure }\DIFdelend \DIFaddbegin \DIFadd{were iteratively adjusted so }\DIFaddend that the $p$\DIFdelbegin \DIFdel{-values }\DIFdelend \DIFaddbegin \DIFadd{-value calculated }\DIFaddend in 2D space \DIFdelbegin \DIFdel{closely match those in the original structure.
This is enabled by optimizing }\DIFdelend \DIFaddbegin \DIFadd{converged to the original PERMANOVA $p$-value.
This was achieved by defining }\DIFaddend an objective function \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{in which }\DIFaddend a new confirmatory term \DIFaddbegin \DIFadd{was }\DIFaddend added to the raw stress,
...
\end{equation}
where the confirmatory term \DIFdelbegin \DIFdel{minimizes }\DIFdelend \DIFaddbegin \DIFadd{minimized }\DIFaddend the discrepancy between \DIFaddbegin \DIFadd{the }\DIFaddend $p$-values from the permutation $F$-distributions obtained \DIFdelbegin \DIFdel{at each dimension}\DIFdelend \DIFaddbegin \DIFadd{in each space}\DIFaddend , namely the original and the two-dimensional \DIFdelbegin \DIFdel{representations (denoted here as }\DIFdelend \DIFaddbegin \DIFadd{(denoted }\DIFaddend $F_\mathbf{x}$ and $F_\mathbf{z}$, respectively).
\DIFdelbegin \DIFdel{Specifically, recognizing that the distributions may depend on dimensionality, we account for this difference by introducing }\DIFdelend \DIFaddbegin \DIFadd{Because the two $F$-ratios were scaled differently, we introduced }\DIFaddend a local regression function $f_\mathbf{z}$ \DIFdelbegin \DIFdel{, which maps between the two }\DIFdelend \DIFaddbegin \DIFadd{to map between them (Algorithm \ref{alg:mapping}). In addition, prior to the regression, each ratio was computed iteratively using a set of permuted labels $\mathbf{y}^{\Pi_k}$ for $k=1, \cdots, K$, and the resulting values were sorted by magnitude. This enabled the original }\DIFaddend $F$\DIFdelbegin \DIFdel{-ratios, each pre-ordered by magnitude according to the permuted labels. In other words, a local regressionis performed using two ordered $F$-ratios, $(F_\mathbf{x}, F_\mathbf{z})$, where each element in the tuple is computed from two independent, permuted sets, $\mathbf{y}^{\Pi_1}$ and $\mathbf{y}^{\Pi_2}$ (Algorithm \ref{alg:mapping}) . Details on }\DIFdelend \DIFaddbegin \DIFadd{-ratio (from the unpermuted labels) to be mapped precisely in the two-dimensional space.
Details of }\DIFaddend the mapping function and the derivation of \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend exact $O_{\rm FMDS}$ are given in \nameref{sec:app1}.

\begin{algorithm}[h]
\caption{\DIFdelbegin \DIFdel{Mapping }\DIFdelend \DIFaddbegin \DIFadd{Regression }\DIFaddend function $f_\mathbf{z}$ \DIFaddbegin \DIFadd{for mapping between $F_\mathbf{x}$ and $F_\mathbf{z}$}\DIFaddend .}
\label{alg:mapping}
\begin{algorithmic}[1]
\Require{
\begin{tabular}{@{}ll}
...
\DIFdelbegin %DIFDELCMD < \Ensure{
%DIFDELCMD < \begin{tabular}{@{}ll}
%DIFDELCMD <     $f_\mathbf{z}:\bbr\rightarrow\bbr$ & : mapping function    
%DIFDELCMD < \end{tabular}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Ensure{
\begin{tabular}{@{}ll}
    $f_\mathbf{z}:\bbr\rightarrow\bbr$ &
\end{tabular}
}
\DIFaddend \State$\mathcal{L}_\mathbf{x} \Leftarrow$ empty list
\State$\mathcal{L}_\mathbf{z} \Leftarrow$ empty list
\DIFdelbegin %DIFDELCMD < \For{$1\leq i \leq 999$}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \For{$1\leq k \leq 999$}
    \DIFaddend \State\DIFdelbegin \DIFdel{$\mathbf{y}^{\Pi_1} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi} \Leftarrow$ }\DIFaddend random permutation on $\mathbf{y}$
    \State\DIFdelbegin \DIFdel{$\mathbf{y}^{\Pi_2} \Leftarrow$ random permutation on $\mathbf{y}$
    }%DIFDELCMD < \State%%%
\DIFdel{$F^{\Pi_1}_\mathbf{x} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi}_\mathbf{x} \Leftarrow$ }\DIFaddend pseudo-$F$ using $\mathbf{D}$ and \DIFdelbegin \DIFdel{$\mathbf{y}^{\Pi_1}$
    }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi}$
    }\DIFaddend \State\DIFdelbegin \DIFdel{$F^{\Pi_2}_\mathbf{z} \Leftarrow$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi}_\mathbf{z} \Leftarrow$ }\DIFaddend pseudo-$F$ using $\mathbf{Z}$ and \DIFdelbegin \DIFdel{$\mathbf{y}^{\Pi_2}$
    }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{y}^{\Pi}$
    }\DIFaddend \State$\mathcal{L}_\mathbf{x} \Leftarrow$ append \DIFdelbegin \DIFdel{$F^{\Pi_1}_\mathbf{x}$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi}_\mathbf{x}$ }\DIFaddend to $\mathcal{L}_\mathbf{x}$
    \State$\mathcal{L}_\mathbf{z} \Leftarrow$ append \DIFdelbegin \DIFdel{$F^{\Pi_2}_\mathbf{z}$ }\DIFdelend \DIFaddbegin \DIFadd{$F^{\Pi}_\mathbf{z}$ }\DIFaddend to $\mathcal{L}_\mathbf{z}$
\EndFor
...
\State$f_\mathbf{z} \Leftarrow$ local regression \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend $\mathcal{L}_\mathbf{x}$ \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend $\mathcal{L}_\mathbf{z}$
\end{algorithmic}
\end{algorithm}


\paragraph*{Majorization algorithm.}
We \DIFdelbegin \DIFdel{implement }\DIFdelend \DIFaddbegin \DIFadd{implemented }\DIFaddend the majorization (or Majorization-Minimization) algorithm to minimize the $F$-MDS objective (Eq \ref{eq:fmds}). First, an exact form of $O_{\rm FMDS}(\mathbf{Z})$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend derived to match the physical dimension of each term (\nameref{sec:app1}),
\begin{equation}
...
\end{equation}
Next, as similarly described in \cite{borg05c, witten11}, the majorization \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend applied to seek an optimal $k$-th point for every $k=1,\cdots N$, while keeping other points fixed:
\begin{equation}
    \mathbf{z}_k^* = \argmin_{\mathbf{z}_k}O_{\rm FMDS}(\mathbf{Z} | \mathbf{z}_1,\cdots \mathbf{z}_{k-1}, \mathbf{z}_{k+1}, \mathbf{z}_N).
    \label{eq:seek_z}
\end{equation}
It can be analytically shown that Eq \ref{eq:seek_z} can be majorized with a quadratic expression in $\mathbf{z}_k$, which is further minimized by taking a derivative with respect to each element of $\mathbf{z}_k$ (denoted as ${z}_{ks},\ s=1,2$). An update rule \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend established by solving for ${z}_{ks}$ after setting the derivative to zero. It should be noted that the update is applied exclusively to the representation where its group-wise difference is \DIFdelbegin \DIFdel{not identified ($p_\mathbf{z}\gg p_\mathbf{x}$), which is }\DIFdelend significant under the original structure ($p_\mathbf{x} < 0.1$)\DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{, but not identified in the two-dimensional embedding ($p_\mathbf{z}\gg p_\mathbf{x}$). The update was designed to continue until when the difference between $p_\mathbf{x}$ and $p_\mathbf{z}$ is minimized. }\DIFaddend Detailed derivations of the expression and the update rule for $\mathbf{Z}$ are described in \nameref{sec:app2}, \DIFdelbegin \DIFdel{and the whole procedure }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend is summarized in Algorithm \ref{alg:mm}.
...
\DIFdelbegin %DIFDELCMD < \For{$1\leq t \leq T$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \State\DIFadd{$p_{\mathbf{x},\,{\rm new}} \Leftarrow$ $p$-value for group difference in $\mathbf{X}$
}\State\DIFadd{$p_{\mathbf{z},\,{\rm new}},\,p_{\mathbf{z},\,{\rm old}} \Leftarrow$ $p$-value for group difference in $\mathbf{Z}$
...
\State\DIFadd{$p_{\mathbf{z},\,{\rm old}} \Leftarrow$ $p$-value for group difference in $\mathbf{Z}$
}\DIFaddend \For{$1\leq k \leq N$}
    ...
    \State$\mathbf{z}_k \Leftarrow$ update $\mathbf{z}_k$ \DIFdelbegin \DIFdel{with $\delta(\mathbf{z})$ }\DIFdelend \DIFaddbegin \DIFadd{using $\delta(\mathbf{Z})$ }\DIFaddend and $f_\mathbf{z}(F_\mathbf{x})$  (Equation S16)
\EndFor
\DIFdelbegin %DIFDELCMD < \EndFor
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \State\DIFadd{$p_{\mathbf{z},\,{\rm new}} \Leftarrow$ $p$-value for group difference in $\mathbf{Z}$
}\EndWhile
...
Experiments were performed using R via RStudio either on Apple M3 chip with 16 GB of RAM, or on 128-thread IBM POWER9 CPU with 256 GB of RAM at the MIT Office of Research Computing and Data. \DIFaddbegin \DIFadd{Datasets are publicly available on Dryad at https://doi.org/10.5061/dryad.vmcvdnd3x.}\DIFaddend \cite{Kim25}
...
Tracking changes in the $F$-MDS representation over training epochs indicated that a minimal hyperparameter value \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{and a stopping rule were }\DIFaddend required for the majorization algorithm to minimize the objective function (Fig \ref{fig:sim_iter}A). \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend semisynthetic datasets generated with SparseDOSSA, we observed a gradual decrease in $p_\mathbf{z}$-value and $O_{\rm FMDS}(\mathbf{Z})$ over successive epochs\DIFdelbegin \DIFdel{(Fig \ref{fig:sim_iter}B), as expected. 
The }\DIFdelend \DIFaddbegin \DIFadd{. 
When training continued without a stopping rule, $p_\mathbf{z}$-values oscillated between 0 and 1, with a higher oscillation rate at larger $\lambda$ (}\nameref{fig:sim_fmds_track}\DIFadd{A). 
The stopping rule was therefore defined to trigger once $p_\mathbf{z}$ approached $p_\mathbf{x}$ closely and their difference was minimized (}\nameref{sec:app2}\DIFadd{). 
Under this rule, the }\DIFaddend algorithm converged in fewer than \DIFdelbegin \DIFdel{25 }\DIFdelend \DIFaddbegin \DIFadd{30 }\DIFaddend iterations when $\lambda\geq0.2$ \DIFdelbegin \DIFdel{. In this setting, termination occurred when $p_\mathbf{z}$ approached $p_\mathbf{x}$ sufficiently closely(e.g., difference $<0.01$). 
This trend was validated by repeating }\DIFdelend \DIFaddbegin \DIFadd{(Fig \ref{fig:sim_iter}B). 
Repeating }\DIFaddend the experiment across \DIFdelbegin \DIFdel{triplicates of }\DIFdelend \DIFaddbegin \DIFadd{triplicate }\DIFaddend simulated datasets of varying sizes \DIFdelbegin \DIFdel{, confirming that $\lambda=0.1$ }\DIFdelend \DIFaddbegin \DIFadd{confirmed that $\lambda=0.15$ }\DIFaddend was the minimal value required for convergence and optimization (\nameref{fig:sim_fmds_track}\DIFaddbegin \DIFadd{B}\DIFaddend ). To preserve convexity of the majorization form of $O_{\rm FMDS}(\mathbf{Z})$ with a positive quadratic coefficient, the hyperparameter was constrained to values not exceeding unity (Fig \ref{fig:sim_iter}C, \nameref{sec:app2}).


\begin{figure}[h!]
    \centering
    \includegraphics[width=5.2in]{submissions/plos/figures/Fig2_rev.png}
    \caption{\textbf{Majorization algorithm optimizes the \textit{F}-informed MDS objective function.} (A) Changes in each term comprising the objective function (Eq \ref{eq:fmds}), including raw stress and confirmatory terms, are plotted against the training epochs for different hyperparameter values $\lambda$. A semisynthetic dataset of $N=200$, replicate 1, was generated using SparseDOSSA \cite{Ma21} \DIFaddbeginFL \hl{(see the ``Semisynthetic data'' section of the Methods)}\DIFaddendFL . (B) The PERMANOVA $p$-value under two-dimensional representation ($p_\mathbf{z}$) is plotted against epoch and $\lambda$. (C) Number of epochs until convergence is plotted against the hyperparameter, ranging between 0.2 and 1. Error bars indicate the standard deviation of triplicates.}
    \label{fig:sim_iter}
\end{figure}
...
The authors acknowledge the MIT Office of Research Computing and Data for providing high performance computing resources that have contributed to the research results reported within this paper. We thank C. Belthangady and J.-Y. Lee for guidance in developing neural network models, and \DIFaddbegin \DIFadd{T. Sapsis and }\DIFaddend R.D. Braatz for \DIFdelbegin \DIFdel{providing additional resources}\DIFdelend \DIFaddbegin \DIFadd{their advice}\DIFaddend .
...
\label{sec:app1}
{\bf \DIFdelbegin \DIFdel{Objective function for computing }\DIFdelend $F$-MDS \DIFaddbegin \DIFadd{objective function and mapping between $F$-ratios}\DIFaddend .}

\paragraph*{S2 Appendix.}
\label{sec:app2}
{\bf Majorization\DIFaddbegin \DIFadd{-Minimization }\DIFaddend algorithm.}

\paragraph*{S3 Appendix.}
\label{sec:app3_rev}
{\bf Hyperparameter selection \DIFaddbegin \DIFadd{procedure}\DIFaddend.}
...
{\bf Visualization of a \DIFdelbegin \DIFdel{binary}\DIFdelend semisynthetic dataset using SparseDOSSA.}

\paragraph*{S2 Fig.}
\label{fig:sim_fmds_track}
{\bf PERMANOVA $p$-values from $F$-informed MDS representations \DIFdelbegin \DIFdel{using semisynthetic data}\DIFdelend \DIFaddbegin \DIFadd{of semisynthetic datasets}\DIFaddend .}
...
\textbf{Hyperparameter selection using grid search.}

\paragraph*{S5 Fig.}
\label{fig:sim_eval_all}
\textbf{Comparison of quality metrics of benchmark ordination methods.}
