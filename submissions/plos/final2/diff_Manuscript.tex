...
Details of the mapping function and the derivation of the exact $O_{\rm FMDS}$ are given in \DIFdelbegin %DIFDELCMD < \nameref{sec:app1} %%%
\DIFdel{Appendix }\DIFdelend \DIFaddbegin \DIFadd{Appendix A, }\nameref{SI} \DIFadd{File}\DIFaddend .
...
We implemented the majorization (or Majorization-Minimization) algorithm to minimize the $F$-MDS objective (Eq \ref{eq:fmds}). First, an exact form of $O_{\rm FMDS}(\mathbf{Z})$ was derived to match the physical dimension of each term (\DIFdelbegin %DIFDELCMD < \nameref{sec:app1} %%%
\DIFdel{Appendix }\DIFdelend \DIFaddbegin \DIFadd{Appendix A, }\nameref{SI} \DIFadd{File}\DIFaddend ),
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:seek_z}
\end{equation}
It can be analytically shown that Eq \ref{eq:seek_z} can be majorized with a quadratic expression in $\mathbf{z}_k$, which is further minimized by taking a derivative with respect to each element of $\mathbf{z}_k$ (denoted as ${z}_{ks},\ s=1,2$). An update rule was established by solving for ${z}_{ks}$ after setting the derivative to zero. It should be noted that the update is applied exclusively to the representation where its group-wise difference is significant under the original structure ($p_\mathbf{x} < 0.1$), but not identified in the two-dimensional embedding ($p_\mathbf{z}\gg p_\mathbf{x}$). The update was designed to continue until when the difference between $p_\mathbf{x}$ and $p_\mathbf{z}$ is minimized. Detailed derivations of the expression and the update rule for $\mathbf{Z}$ are described in \DIFdelbegin %DIFDELCMD < \nameref{sec:app2} %%%
\DIFdel{Appendix , }\DIFdelend \DIFaddbegin \DIFadd{Appendix B, }\nameref{SI} \DIFadd{File, }\DIFaddend which is summarized in Algorithm \ref{alg:mm}.
...
\DIFdel{Appendix }\DIFdelend \DIFaddbegin \DIFadd{Appendix D, }\nameref{SI} \DIFadd{File}\DIFaddend .
...
To preserve convexity of the majorization form of $O_{\rm FMDS}(\mathbf{Z})$ with a positive quadratic coefficient, the hyperparameter was constrained to values not exceeding unity (Figs \ref{fig:sim_iter}C and Appendix B, \nameref{SI} File).
...
\DIFdel{Appendix }\DIFdelend \DIFaddbegin \DIFadd{Appendix E, }\nameref{SI} \DIFadd{File}\DIFaddend ). 
...
\DIFdelbegin %DIFDELCMD < \bibliography{submissions/plos/final/Manuscript}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{submissions/plos/final2/Manuscript}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \label{sec:app1}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{SI}
\DIFaddend {\bf \DIFdelbegin \DIFdel{Appendix. $F$-MDS objective function and mapping between $F$-ratios}\DIFdelend \DIFaddbegin \DIFadd{File. Supplementary Appendices}\DIFaddend .}

\DIFdelbegin \paragraph*{\DIFdel{S2}}%DIFAUXCMD
\DIFdel{\hspace{-2ex}
}%DIFDELCMD < \label{sec:app2}
%DIFDELCMD < {\bf %%%
\DIFdel{Appendix. Majorization-Minimization algorithm.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \paragraph*{S1}\hspace{-2ex}
%DIF >  \label{sec:app1}
%DIF >  {\bf Appendix. $F$-MDS objective function and mapping between $F$-ratios.}

\DIFdelbegin \paragraph*{\DIFdel{S3}}%DIFAUXCMD
\DIFdel{\hspace{-2ex}
}%DIFDELCMD < \label{sec:app3_rev}
%DIFDELCMD < {\bf %%%
\DIFdel{Appendix. Hyperparameter selection procedure.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \paragraph*{S2}\hspace{-2ex}
%DIF >  \label{sec:app2}
%DIF >  {\bf Appendix. Majorization-Minimization algorithm.}

\DIFdelbegin \paragraph*{\DIFdel{S4}}%DIFAUXCMD
\DIFdel{\hspace{-2ex}
}%DIFDELCMD < \label{sec:app4_rev}
%DIFDELCMD < {\bf %%%
\DIFdel{Appendix. Human gut microbiome dataset.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \paragraph*{S3}\hspace{-2ex}
%DIF >  \label{sec:app3_rev}
%DIF >  {\bf Appendix. Hyperparameter selection procedure.}

\DIFdelbegin \paragraph*{\DIFdel{S5}}%DIFAUXCMD
\DIFdel{\hspace{-2ex}
}%DIFDELCMD < \label{sec:app5_rev}
%DIFDELCMD < {\bf %%%
\DIFdel{Appendix. Neural network model and architecture.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \paragraph*{S4}\hspace{-2ex}
%DIF >  \label{sec:app4_rev}
%DIF >  {\bf Appendix. Human gut microbiome dataset.}

\DIFdelbegin \paragraph*{\DIFdel{S6}}%DIFAUXCMD
\DIFdel{\hspace{-2ex}
}%DIFDELCMD < \label{sec:app6_rev}
%DIFDELCMD < {\bf %%%
\DIFdel{Appendix. Computational complexity.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \paragraph*{S5}\hspace{-2ex}
%DIF >  \label{sec:app5_rev}
%DIF >  {\bf Appendix. Neural network model and architecture.}

%DIF >  \paragraph*{S6}\hspace{-2ex}
%DIF >  \label{sec:app6_rev}
%DIF >  {\bf Appendix. Computational complexity.}
\DIFaddbegin 

\DIFaddend \paragraph*{S1}\hspace{-2ex}
\label{fig:sim_data_vis}
{\bf Fig. Visualization of a semisynthetic dataset using SparseDOSSA.} \DIFaddbegin \DIFadd{(A) Heatmap of its log-scaled relative abundance by its microbial taxa and sample number. (B) Density histogram of the data and (C) principal coordinates analysis (PCoA) and PERMANOVA $p$-values based on PCoA results with Euclidean distance ($p_\mathbf{z}$) and original structure with Bray-Curtis dissimilarity ($p_\mathbf{x}$). $N=200$ samples were generated.
}\DIFaddend 

\paragraph*{S2}\hspace{-2ex}
\label{fig:sim_fmds_track}
{\bf Fig. Stabiliity and trajectories of PERMANOVA $p$-values from $F$-MDS representations.} \DIFaddbegin \DIFadd{(A) Trajectories of $p_z$ across 50 epochs for $\lambda = 0.2, 0.5,$ and $0.8$. Red dashed lines indicate the region of validity ($|p_z - p_x| < 0.05$). Pink points highlight where the stopping rule triggers, capturing the optimal statistical alignment before further oscillations occur. (B) Trajectories of $p$-values with the stopping rule ($|p_z - p_x| < 0.05$). Heatmaps show $p_z$ values plotted against training epochs for hyperparameters $\lambda \in [0,1]$. Triplicate datasets of varying sizes ($N = 50, 100, 200, 500$) were evaluated to demonstrate consistent stabilization across conditions.
}\DIFaddend 

\paragraph*{S3}\hspace{-2ex}
\label{fig:sim_fmds_smds}
\textbf{Fig. Pairwise distance analyses from $F$-informed MDS and supervised MDS \DIFaddbegin \DIFadd{using semisynthetic data}\DIFaddend .} \DIFaddbegin \DIFadd{The plots are titled with the dataset size $N$ and compared across different methods with hyperparameter values as follows: $\lambda$, $F$-MDS; $\alpha$, superMDS. After calculating pairwise distances with Bray-Curtis dissimilarity (original) and Euclidean (2D representation), (B) their Pearson correlation coefficient and (C) normalized stress (Stress-1) were obtained. Error bars are standard deviation of triplicates.
}\DIFaddend 

\paragraph*{S4}\hspace{-2ex}
\label{fig:fmds_gridsearch}
\textbf{Fig. Hyperparameter selection using grid search.} \DIFaddbegin \DIFadd{An objective function $f_{\rm obj}(\lambda)$ was defined to simultaneously reflect the number of training epochs and the preservation of the original structure (see Equation S17). (A) For each semisynthetic dataset size $N$, $f_{\rm obj}(\lambda)$ is plotted against the hyperparameter $\lambda$. The minimum value of $f_{\rm obj}(\lambda)$ is highlighted in red. (B) The optimal hyperparameter $\lambda_{\rm min}$ is plotted against dataset size $N$, with each dataset represented by a symbol.
}\DIFaddend 

\paragraph*{S5}\hspace{-2ex}
\label{fig:sim_eval_all}
\textbf{Fig. Comparison of quality metrics of benchmark ordination methods.} \DIFaddbegin \DIFadd{Trustworthiness and continuity to evaluate (A) Local structural preservation is assessed using trustworthiness and continuity. (B) Global structural preservation is similarly evaluated with trustworthiness and continuity. (C) Global distortion is quantified by Stress-1 and Pearson correlation of Shepard diagrams. (D) Preservation of statistical inference is measured by the $F$-rank-ratio and $F$-correlation using randomly permuted label sets. The following hyperparameters were used for each method: $\lambda$ for $F$-MDS, number of neighbors $n$ for UMAP (both supervised (-S) and unsupervised (-U)), perplexity (perp) for t-SNE, and the number of shortest dissimilarities $n$ for Isomap. Error bars represent the standard deviation across triplicate measurements.
}\DIFaddend 

\paragraph*{S6}\hspace{-2ex}
\label{fig:shepard_alga_all}
\textbf{Fig. Shepard plot from algal microbiome dataset with eight ordination methods.} \DIFaddbegin \DIFadd{The plots are titled with the respective method and hyperparameter values as follows: $\lambda$, $F$-MDS; $\alpha$, superMDS; Nearest neighbors number, supervised (-S) or unsupervised (-U) UMAP; Perplexity, t-SNE; Shortest dissimilarities number, Isomap; none, neural network (NN). X- and Y-axis denote distances in the original and embedding dimensions, respectively.
}\DIFaddend 

\paragraph*{S7}\hspace{-2ex}
\label{fig:f_corr_alga_all}
\textbf{Fig. \textit{F}-correlation plot from algal microbiome dataset.} \DIFaddbegin \DIFadd{Pseudo $F$-ratios comparing the original dimension (x-axis) and from eight dimension reduction methods (y-axis) with algal microbiome data. Pseudo $F$'s were calculated by randomly permuting labels by 500 times. Highlighted with red denotes the location of $F$'s from unpermuted labels. Each plot is titled with the method and hyperparameter used.
}\DIFaddend 

\paragraph*{S8}\hspace{-2ex}
\label{fig:fmds_rep_analysis}
\textbf{Fig. Cluster centroid and variances of $F$-MDS representations.} \DIFaddbegin \DIFadd{Cluster centroids and variances of $F$-MDS representations are shown for semisynthetic datasets of size $N=50$ (A–C), $N=100$ (D–F), $N=200$ (G–I), $N=500$ (J–L) and for the algal microbiome (M–O). The first column in each row displays the distance between group centroids. The second and third columns show the variance of each group, measured along the long and short principal axes, respectively. For the variance panels, blue and red colors denote groups 1 and 2. Error bars represent the standard deviation across triplicate measurements.
}\DIFaddend 

\paragraph*{S9}\hspace{-2ex}
\label{fig:sim_ternary_vis}
\textbf{Fig. Visualization of a ternary semisynthetic dataset.} \DIFaddbegin \DIFadd{(A) Heatmap of its log-scaled relative abundance by its microbial taxa and sample number. (B) Density histogram of the data and (C) principal coordinates analysis (PCoA) and PERMANOVA $p$-values based on PCoA results with Euclidean distance ($p_\mathbf{z}$) and original structure with Bray-Curtis dissimilarity ($p_\mathbf{x}$). $N=75$ semisynthetic data were generated.
}\DIFaddend 
