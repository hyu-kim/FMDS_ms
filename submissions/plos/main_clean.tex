...
Exploratory analysis of microbiome data \cite{peterson24, Armstrong22}, after preprocessing the sequencing reads into with data normalization \cite{Lin20, Weiss17, McMurdie14, paulson13}, typically begins with visualizing the multivariate structure to identify data patterns, distinguish variations, or remove outliers.
...
However, a standard $F$-test assumes that observations are normally distributed with equal variance, an assumption that does not hold for most compositional and/or zero-inflated data.
...
Specifically, recognizing that the $F$-distributions may depend on dimensionality, we account for this difference by introducing a regression function $f_\mathbf{z}$, which maps between the two $F$-ratios, each pre-ordered by magnitude after computing $F$ using permuted labels.
In other words, a local regression is performed using two ordered $F$-ratios, $(F_\mathbf{x}^{\Pi}, F_\mathbf{z}^{\Pi})$, where each element in the tuple is computed from a permuted labels set, $\mathbf{y}^{\Pi}$ (Algorithm \ref{alg:mapping}). 
It is important to note, that procedure of ordering $F^{\Pi}$ by its size allows to draw significance for the original $F$-ratio and ultimately evaluate the projection method based on the $p$-value.
Details on the mapping function and the derivation of an exact $O_{\rm FMDS}$ are given in \nameref{sec:app1}.
...
\paragraph*{Semisynthetic data.} \label{par:semisyn}
...
\paragraph*{Real microbiome community.} \label{par:real}
...
\begin{figure}[h!]
    ...
    \caption{\textbf{Majorization algorithm optimizes the \textit{F}-informed MDS objective function.} (A) Changes in each term comprising the objective function (Eq \ref{eq:fmds}), including raw stress and confirmatory terms, are plotted against the training epochs for different hyperparameter values $\lambda$. A semisynthetic dataset of $N=200$, replicate 1, was generated using SparseDOSSA \cite{Ma21} (\nameref{par:semisyn}). (B) The PERMANOVA $p$-value under two-dimensional representation ($p_\mathbf{z}$) is plotted against epoch and $\lambda$. (C) Number of epochs until convergence is plotted against the hyperparameter, ranging between 0.2 and 1. Error bars indicate the standard deviation of triplicates.}
    \label{fig:sim_iter}
\end{figure}
...
The proposed approach has practical value in any setting where researchers rely on PERMANOVA for statistical inference and MDS for visual interpretation, yet the two yield conflicting conclusions. A concrete example arises in microbiome research, where PERMANOVA often identifies significant differences between bacterial communities—such as distinctions associated with host identity or disease status, while standard MDS plots fail to visually separate these groups. Our $F$-informed MDS resolves this mismatch by producing low-dimensional representations that clearly reflect the statistically significant group differences, allowing practitioners to more confidently interpret biologically meaningful patterns.

Comparable real-world needs appear in metabolomics and proteomics (e.g., distinguishing healthy versus diseased metabolic profiles; \cite{ren2015computational, steuer2007gentle}), neuroimaging connectivity studies (e.g., separating patient subtypes based on brain network structure; \cite{shinkareva2013examining}), and chemometrics \cite{chemometrics} or industrial quality control. In all of these contexts, high-dimensional measurements often contain class-discriminating signals that reside in later dimensions and are therefore overlooked by standard MDS. By prioritizing the preservation of group differences, $F$-informed MDS offers interpretable visualizations that more closely align with statistical results, supporting clearer and more reliable decision-making even when this involves a modest trade-off in preserving the original distance structure.
