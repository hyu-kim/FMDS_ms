...
We defined the objective function $O_{\rm FMDS}(\mathbf{Z})$ from the MDS stress function by adding a confirmatory term so that PERMANOVA $p$-values are close to each other when the function is minimized.
First, a local regression function $f_\mathbf{z}(F_\mathbf{x})$ was designed to minimize a difference in the $p$-values testing group differences under the original $S$- and two-dimensional embedding through the confirmatory term (see Eq 5 of the main text).
...
In specific, for a given label group set $[y_i]_{i=1}^{N}$ with the number of groups $G$, the pseudo $F$-statistics are expressed as
\begin{equation}
\begin{aligned}
F^\Pi_\mathbf{x} &= 
...
\end{aligned}
\end{equation}
where superscript $\Pi$ denoting the permutation. 
